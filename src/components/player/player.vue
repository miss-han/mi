<template>
  <div class="player" v-show="playlist.length > 0">
    ...
    <!--url-->
    <audio ref="audio" :src="url" @canplay="ready"
...
export default {
  ...
  computed: {
    ...
    iconMode() {
      return this.mode ===
        playMode.sequence ? 'icon-sequence' : this.mode ===
  ...
  methods: {
    ...
    _getMusic() {
      getMusic(this.currentSong.mid).then((res) => {
        let r = res.data.items[0]
        this.url = `http://dl.stream.qqmusic.qq.com/${r.filename}?vkey=${r.vkey}&guid=3598100672&uin=0&fromtag=66`
      })
    },
    // 播放也开关
    back() {
    ...
    changeMode() {
      ...
      if (mode === playMode.random) {
        // console.log(shuffle(this.sequenceList))
